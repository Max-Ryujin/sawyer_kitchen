...
def main(args):
    ...
    wandb_cfg.update({
        "seed": args.seed,
        "example_obs_shape": getattr(example_transition['observations'], 'shape', None),
    ...
    save_dir = os.path.join(args.save_dir, args.wandb_name) if args.wandb_name else args.save_dir
    ...
    for step in range(1, args.train_steps + 1):
        ...
        if step < args.seed_steps:
            ...
            action = agent.sample_actions(observations=ob[None], seed=key, temperature=0.0)
        ...
        if step % 10 == 0 and _wandb_run is not None:
            log_dict = {f'training/{k}': _to_scalar(v) for k, v in info.items()}
...
if __name__ == "__main__":
    ...
    p.add_argument("--train-steps", type=int, default=100000, help="Number of training steps")
    p.add_argument("--seed-steps", type=int, default=1000, help="Number of seed exploration steps")
    p.add_argument("--save-interval", type=int, default=10000, help="Save checkpoint interval")
    p.add_argument("--save-dir", type=str, default="tmp/sac_checkpoints", help="Directory to save checkpoints")
    p.add_argument("--batch-size", type=int, default=256, help="Training batch size")
    p.add_argument("--wandb-project", type=str, default="kitchen", help="wandb project name")
